# Changelog

## [0.1.3a4](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.3a4) (2026-09-18)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.3a3...0.1.3a4)

**Merged pull requests:**

- Update docker/metadata-action action to v6 [\#34](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/34) ([renovate[bot]](https://github.com/apps/renovate))

## [0.1.3a3](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.3a3) (2026-09-18)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.3a2...0.1.3a3)

**Merged pull requests:**

- Update docker/login-action action to v4 [\#32](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/32) ([renovate[bot]](https://github.com/apps/renovate))

## [0.1.3a2](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.3a2) (2026-09-18)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.3a1...0.1.3a2)

**Merged pull requests:**

- Update docker/build-push-action action to v7 [\#31](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/31) ([renovate[bot]](https://github.com/apps/renovate))

## [0.1.3a1](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.3a1) (2026-09-18)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.2a1...0.1.3a1)

**Merged pull requests:**

- fix: run the licence check on a Python that deltachat ships a wheel for [\#39](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/39) ([openvoiceos-bot](https://github.com/openvoiceos-bot))

## [0.1.2a1](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.2a1) (2026-09-01)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.1a6...0.1.2a1)

**Merged pull requests:**

- fix\(deps\): raise floor pins to the proven HiveMind stack; drop stale caps [\#37](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/37) ([JarbasAl](https://github.com/JarbasAl))

## [0.1.1a6](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.1a6) (2026-08-15)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.1a3...0.1.1a6)

**Merged pull requests:**

- Update actions/checkout action to v7 [\#27](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/27) ([renovate[bot]](https://github.com/apps/renovate))

## [0.1.1a3](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.1a3) (2026-08-15)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.1a5...0.1.1a3)

## [0.1.1a5](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.1a5) (2026-08-15)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.1a4...0.1.1a5)

## [0.1.1a4](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.1a4) (2026-08-15)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.1a2...0.1.1a4)

**Merged pull requests:**

- docs: add AGENTS.md with per-repo agent conventions [\#23](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/23) ([JarbasAl](https://github.com/JarbasAl))
- ci: test, build and publish the container image [\#21](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/21) ([JarbasAl](https://github.com/JarbasAl))
- Add Dockerfile [\#19](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/19) ([JarbasAl](https://github.com/JarbasAl))
- docs: make the hub allow-msg whitelist step explicit [\#18](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/18) ([JarbasAl](https://github.com/JarbasAl))

## [0.1.1a2](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.1a2) (2026-08-15)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.1a1...0.1.1a2)

**Merged pull requests:**

- Bound the async client's HiveMind handshake retries [\#24](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/24) ([JarbasAl](https://github.com/JarbasAl))

## [0.1.1a1](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.1a1) (2026-08-14)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.0a2...0.1.1a1)

**Merged pull requests:**

- fix: raise hivemind-bus-client floor to \>=1.0.13a1, drop \<1.0.0 cap [\#20](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/20) ([JarbasAl](https://github.com/JarbasAl))

## [0.1.0a2](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.0a2) (2026-07-31)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.1.0a1...0.1.0a2)

**Merged pull requests:**

- docs: rewrite README in Simplified Technical English [\#16](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/16) ([JarbasAl](https://github.com/JarbasAl))

## [0.1.0a1](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.1.0a1) (2026-07-05)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.0.3a2...0.1.0a1)

**Merged pull requests:**

- feat: migrate to AsyncHiveMessageBusClient \(composition over inheritance\) [\#7](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/7) ([JarbasAl](https://github.com/JarbasAl))

## [0.0.3a2](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.0.3a2) (2026-07-04)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/0.0.3a1...0.0.3a2)

**Merged pull requests:**

- test: hivescope e2e + CI [\#13](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/13) ([JarbasAl](https://github.com/JarbasAl))

## [0.0.3a1](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/tree/0.0.3a1) (2026-06-23)

[Full Changelog](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/compare/b818cda5959c554659d330937da0c459885f3d10...0.0.3a1)

**Merged pull requests:**

- fix!: modernize packaging, repair console entry point, add CI + tests [\#11](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/11) ([JarbasAl](https://github.com/JarbasAl))
- docs: zero-to-hero README and docs [\#8](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/8) ([JarbasAl](https://github.com/JarbasAl))
- Update actions/setup-python action to v6 [\#5](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/5) ([renovate[bot]](https://github.com/apps/renovate))
- Update actions/checkout action to v6 [\#3](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/3) ([renovate[bot]](https://github.com/apps/renovate))
- Configure Renovate [\#1](https://github.com/JarbasHiveMind/HiveMind-deltachat-bridge/pull/1) ([renovate[bot]](https://github.com/apps/renovate))



\* *This Changelog was automatically generated by [github_changelog_generator](https://github.com/github-changelog-generator/github-changelog-generator)*
