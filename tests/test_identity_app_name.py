...
def test_the_bridge_asks_for_its_own_identity_and_hands_it_to_the_bridge():
    import asyncio
    from unittest.mock import AsyncMock
    from hm_deltachat_bridge import __main__ as entry
    identity = _identity()
    with patch.object(entry, "NodeIdentity", return_value=identity) as ctor, \
         patch.object(entry, "HiveMindDeltaChatBridge") as bridge:
        bridge.return_value.start = AsyncMock(side_effect=_Stop)
        bridge.return_value.stop = AsyncMock()
        with pytest.raises(_Stop):
            asyncio.run(entry._amain("a@b", "pw", "", "", "", 0))
    ctor.assert_called_once_with(app_name=APP_NAME)
    assert bridge.call_args.kwargs.get("identity") is identity, (
        "the bridge was built without the application's identity")
